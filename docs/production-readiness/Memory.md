...
- The corrected PTFE target environment is running on isolated port 3103 and its checkout was refreshed after the first controlled browser job succeeded. Empty-sheet guards, disposable migrations, one-attempt Master Log and Job x Job outbox delivery, full synthetic cleanup, web/worker readiness, and production-port preservation passed. Two focus-based quantity-entry fixes were ineffective in the target Edge session. The current source solution no longer renders zero as the input value: Time worked, Start quantity, and End quantity use empty values with a visual zero placeholder, and their spinner arrows are removed. Browser regression confirmation, remaining UAT, rollback, and final Stop cleanup remain.
