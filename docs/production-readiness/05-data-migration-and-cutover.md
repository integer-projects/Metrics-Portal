...
The supervised PTFE production cutover then passed at 09:39 ET on August 17, 2026. A fresh verified off-machine backup and timestamped rollback environment were created before enabling only the PTFE server-session and database-submission flags. The web process, worker, readiness, PL/PTFE/PI feature boundaries, and five-minute monitor passed after restart. The controlled production workflow stored one PTFE job and one event in the Master Log path, retained both in the server shift, captured two separate Job x Job rows at End Shift, and returned to login. All four database and outbox records reached `submitted` on their first attempt with production Smartsheet row IDs; the unfinished/failed and duplicate-remote-row queries returned zero rows. Keep the compatibility PTFE implementation and rollback environment available through the observation window.
