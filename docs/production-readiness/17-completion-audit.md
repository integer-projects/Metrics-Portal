...
| PTFE target UAT and rollback | Ready to execute | `Manage-PtfeUatEnvironment.ps1` creates an isolated checkout, port 3103, database, web process, worker, and two test destinations; it provides guarded Start, Rollback, and Stop actions. The first target Start proved empty test sheets and isolated migration, then exposed and safely stopped at a destination-safety variable mismatch before the portal started. | Apply the merged safety fix, run guarded Stop against the retained failed state, rerun Start, complete named associate/supervisor browser UAT and failure scenarios, rehearse Rollback, then run Stop and retain the evidence. |
