...
| PTFE production cutover | Not ready | UAT, rollback, named approval, and the healthy production monitor passed; cutover procedures are documented and the feature flags default off. | Requires production destination expansion, fresh backup, supervised first job/event/End Shift, and reconciliation in both destinations. |
...
| Local operations monitoring | Complete | The five-minute task is installed on the target. Its August 17 proof returned result `0` with both PM2 processes, PostgreSQL, liveness/readiness, queue, backup task/freshness, and disk healthy; output is atomic structured JSON. | Feed the result to an approved routed-alert transport when that external decision is made. |
...
1. During an approved change window, add `Submission ID` to both PTFE production destinations and rerun the exact contract validators.
2. Take a fresh verified backup, pull the approved release, enable PTFE flags in the supervised window, and reconcile the first production job, event, Pull or low-yield rule, and End Shift across PostgreSQL, both outboxes, and both Smartsheets.
3. Observe PTFE before starting PI. Continue PL observation and close its support handoff in parallel.
4. Close the external TLS/DNS, routed-alert, retention-policy, permanent-owner, and quarterly-drill gates before declaring the overall program production ready.

## Current Stop Conditions

Do not enable PTFE production flags yet. Technical target-server UAT, rollback, named department approval, and production monitor proof are complete, but both production `Submission ID` columns remain. Do not start PI implementation until PTFE production acceptance establishes the reusable department pattern. Do not schedule backup deletion until the storage-policy gate is closed.
